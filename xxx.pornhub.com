...
StatsHealthBackground.Name = "PORN.XXX"
...
TargetAimbotSec:AddTextbox("Prediction", nil, function(Value)
	TargetAimbot.Prediction = Value or 0
	TargetAimbot.RealPrediction = Value or 0
...
CameraAimbotSec:AddTextbox("Prediction", nil, function(Value)
	CameraAimbot.Prediction = Value or 0
	CameraAimbot.RealPrediction = Value or 0
...
local TrashTalkTog = UtilitiesSec:AddToggle("Trash Talk", false, function(Value)
	if Value then
		local TrashTalkWords = {"gg/halalgaming", "gg/hUvujCnGMb", "How to aim pls help", "wow my little brother was playing and beat you :rofl:", "Mobile player beat u Lol XD", "420 ping and u got SLAMMED", "ur bad", "seed", "im not locking ur just bad", "clown", "sonned", "LOLL UR BAD", "dont even try.. ur not enough for the alpha", "ez", "gg = get good", "my grandmas better than u :skull:", "hop off kid", "bro cannot aim", "u got absolutely DOGGED on", "i run this server son", "what is bro doing :skull:", "no way u lost that"}
...
RunService.RenderStepped:Connect(function()
	if TargetAimbot.Enabled and TargBindEnabled and TargetAimbot.AutoPred then
		TargetAimbot.RealPrediction = math.round(Stats.Network.ServerStatsItem["Data Ping"]:GetValue()) / 225 * 0.1 + 0.1
	else
...
RunService.RenderStepped:Connect(function()
	if CameraAimbot.Enabled and CamBindEnabled and CameraAimbot.AutoPred then
		CameraAimbot.RealPrediction = math.round(Stats.Network.ServerStatsItem["Data Ping"]:GetValue()) / 225 * 0.1 + 0.1
	else
